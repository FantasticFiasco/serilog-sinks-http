...
You will have to migrate your code if you're using `DurableHttpUsingTimeRolledBuffers`, i.e. use the sink with a rolling behavior defined by a time interval. The parameter `bufferPathFormat` has been renamed to `bufferBaseFileName` and the parameter `bufferRollingInterval` has been added.
